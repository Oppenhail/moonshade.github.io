...
import { Wrench, FolderArchive } from 'lucide-react';
...
const defaultTile = (i) => ({ id: uuid(), label: `Tile ${i + 1}`, emoji: '', color: '', blocked: false, note: '', icon: null, })
...
export default function App() {
  ...
  function insertTile(atIndex) {
    updateCurrent(ev => {
      const tiles = ev.board.tiles;
      const newTile = defaultTile(atIndex);
      tiles.splice(atIndex, 0, newTile);
      for (let i = 0; i < tiles.length; i++) {
        tiles[i].label = tiles[i].label || `Tile ${i + 1}`;
      }
      const size = ev.board.rows * ev.board.cols;
      for (const t of Object.values(ev.teams)) {
        t.position = clamp(t.position ?? 0, 0, size - 1);
      }
    });
  }
  function removeTileAt(idx) {
    updateCurrent(ev => { ev.board.tiles.splice(idx, 1); });
  }
  function moveTileIndex(from, to) {
    updateCurrent(ev => {
      const arr = ev.board.tiles;
                  ...
                  <DropdownMenuContent align="end" className="w-80 p-2">
                    <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <Button variant="outline" onClick={() => insertTile(idx)}>Insert Before</Button>
                        <Button variant="outline" onClick={() => insertTile(idx + 1)}>Insert After</Button>
                        <Button variant="outline" onClick={() => moveTileIndex(Math.max(0, idx - 1), idx)}>Move Up</Button>
                        <Button variant="outline" onClick={() => moveTileIndex(Math.min(board.tiles.length - 1, idx + 1), idx)}>Move Down</Button>
                        <Button variant="ghost" onClick={() => removeTileAt(idx)}>
                          <Trash2 className="h-4 w-4 mr-2" />Delete Tile
                        </Button>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        <input id={`blocked-${idx}`} type="checkbox"
                          checked={tile.blocked}
                          onChange={(e) => paintTile(idx, { blocked: e.target.checked })} />
                      ...
                      <div>
                        <Label>Icon (png/jpg/jpeg/webp)</Label>
                        <input
                          type="file"
                          ...
                          onChange={async (e) => {
                            ...
                            reader.onload = () => {
                              paintTile(idx, { icon: { name: file.name, dataURL: reader.result, ext } });
                            };
                            reader.readAsDataURL(file);
                          }}
                        />
                        {tile.icon && <small className="block opacity-80 mt-1">{tile.icon.name}</small>}
                      </div>
  ...
  function EventsPanel() {
    function exportJSON() {
      ...
      a.href = url; a.download = `${data.eventName}_board.json`; a.click();
    ...
    async function exportZIP() {
      const data = serializeForBot();
      const zip = new JSZip();
      zip.file('board.json', JSON.stringify(data.board, null, 2));
      const icons = zip.folder('icons');

      for (const t of data.board) {
        if (t.iconFile && t.__iconDataURL) {
          const res = await fetch(t.__iconDataURL);
          const buf = await res.arrayBuffer();
          icons.file(t.iconFile, buf);
        }
      }
      const blob = await zip.generateAsync({ type: 'blob' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url; a.download = `${data.eventName}_board_package.zip`; a.click();
      URL.revokeObjectURL(url);
    }
    function serializeForBot() {
      const ev = current;
      const tiles = [];
      // Flatten grid order to 1..N (left->right, top->bottom)
      for (let i = 0; i < ev.board.tiles.length; i++) {
        const t = ev.board.tiles[i];
        ...
        if (t.icon) {
          const ext = '.' + ((t.icon.ext || 'png').replace(/^\./, ''));
          out.iconFile = `${t.id}${ext}`;
          // keep data URL temporarily for zip export
          out.__iconDataURL = t.icon.dataURL;
        }
        tiles.push(out);
      }
      return { eventName: ev.name, board: tiles };
    }
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" onClick={exportJSON}>
            <Download className="h-4 w-4 mr-2" />Export Board JSON
          </Button>
          <Button variant="outline" onClick={exportZIP}>
            <FolderArchive className="h-4 w-4 mr-2" />Export ZIP (board+icons)
          </Button>
          <BotBridge serializeForBot={()=>serializeForBot()} />
        </div>
        <div className="grid md:grid-cols-2 gap-4">
